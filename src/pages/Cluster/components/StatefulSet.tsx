...
const StatefulSet: React.FC<StatefulSetProps> = ({ name, namespace }) => {
  ...
  const get_resource = async () => {
    ...
    if (code == 200) {
      const { statefulSets, listMeta } = data;
      setCount(listMeta.totalItems);
      setData(statefulSets);
    }
  };
  ...
  const columns = [
    {
      ...
      render: (name: string, record: any) => (
        <Link
          to={`/apps/info/statefulset/${record.objectMeta.namespace}/${name}`}
        >
          {name}
        </Link>
      ),
